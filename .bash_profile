...
if [ $1 == "get" ];
then
	python3 ~/dev/python/mimir/mimir.py -$1 -pl $2 -pr $3 -subs
else
	python3 ~/dev/python/mimir/mimir.py -$1 -pl $2 -pr $3 -url $4 -src manual
fi
}

check_alive(){
~/dev/scripts/check_alive.sh $1
}

way(){
echo $1 | ~/tools/go/bin/waybackurls

}


httprobe(){
echo $1 | ~/tools/go/bin/httprobe | ruby -e 'STDIN.read.lines.sort!.reverse!.uniq! { |x| x.split(%r{https?://}).last }.each { |x| puts x }'
}

reload_bash(){
cp ~/dev/bash/recon_profile/.bash_profile ~/
}
